dnl The autoconf macro AC_FUNC_MMAP tests for a mmap that supports
dnl (the optional) MAP_FIXED, but we don't need that. So test
dnl if mmap supports MAP_SHARED instead.

# GGI_FUNC_MMAP_FILEIO
# --------------------
AC_DEFUN([GGI_FUNC_MMAP_FILEIO],
[
AC_CHECK_HEADERS(stdlib.h unistd.h)
AC_CHECK_FUNCS(getpagesize)
AC_CACHE_CHECK([for working mmap file i/o],
[ggi_cv_func_mmap_fileio],
[AC_RUN_IFELSE(
...
main ()
{
  ...
  [ggi_cv_func_mmap_fileio=yes],
  [ggi_cv_func_mmap_fileio=no],
  dnl assume it doesn't work when cross-compiling
  [ggi_cv_func_mmap_fileio=yes])])

if test "$ggi_cv_func_mmap_fileio" = yes; then
