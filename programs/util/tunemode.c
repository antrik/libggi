...
{ char buffer[1024],*token;
  ...
  { fgets(buffer,1024,infile);
    ...
    { token=strtok(NULL,"\" ");verb(1,"Loading Mode: %s\n");
      ...
      prefmode[pref_num].x.width    =atoi(token=strtok(NULL," \t\n"));verb(2,"%s ");
      prefmode[pref_num].x.blankstart=
      prefmode[pref_num].x.syncstart=atoi(token=strtok(NULL," \t\n"));verb(2,"%s ");
      prefmode[pref_num].x.blankend=
      prefmode[pref_num].x.syncend  =atoi(token=strtok(NULL," \t\n"));verb(2,"%s ");
      prefmode[pref_num].x.total    =atoi(token=strtok(NULL," \t\n"));verb(2,"%s  ");
      prefmode[pref_num].y.width    =atoi(token=strtok(NULL," \t\n"));verb(2,"%s ");
      prefmode[pref_num].y.blankstart=
      prefmode[pref_num].y.syncstart=atoi(token=strtok(NULL," \t\n"));verb(2,"%s ");
      prefmode[pref_num].y.blankend=
      prefmode[pref_num].y.syncend  =atoi(token=strtok(NULL," \t\n"));verb(2,"%s ");
      prefmode[pref_num].y.total    =atoi(token=strtok(NULL," \t\n"));verb(2,"%s\n");
...
{ int mx,my,x;  
  ...
  for(x=(mx=(widthx/2))%50;x<widthx-1;x+=50)
    if (x>0) ggiDrawVLine(vis,x,1,widthy-2);
  for(x=(my=(widthy/2))%50;x<widthy-1;x+=50)
    if (x>0) ggiDrawHLine(vis,1,x,widthx-2);
  ...
  ggiPrintf(vis,10,my+ 1,"        wdth blnk sync send bend total");
  ggiPrintf(vis,10,my+17,"Xsync%c: %4d %4d %4d %4d %4d %4d",
  	     pp->x.polarity>0 ? '+' : '-',
             pp->x.width,pp->x.blankstart,pp->x.syncstart,
             pp->x.syncend,pp->x.blankend,pp->x.total);
  ggiPrintf(vis,10,my+33,"Ysync%c: %4d %4d %4d %4d %4d %4d",
  	     pp->y.polarity>0 ? '+' : '-',
             pp->y.width,pp->y.blankstart,pp->y.syncstart,
             pp->y.syncend,pp->y.blankend,pp->y.total);
  ggiPrintf(vis,10,my+49,"Freq  : %5.3fMhz/%5.2fkHz/%5.2fHz",
  	     pp->dclk/1000000.0,
             0.001*pp->dclk/pp->x.total,
...
{   int y;
    ...
    oldmode.x.width=wantx;oldmode.y.width=wanty;
    if (ggi_ioctl(MONITOR_GETPRESETMODE,&oldmode)) { perror("getpresetmode : "); }
    trymode=oldmode;
    while(1)
    { 
      if (ggi_ioctl(MONITOR_SETPRESETMODE,&trymode)) { perror("setpresetmode : "); }

#ifdef DEBUG
      printf("trymode: x: %d %d %d %d %d %d\n"
	       "         y: %d %d %d %d %d %d\n",
	       trymode.x.width,trymode.x.blankstart,trymode.x.syncstart,
	       			trymode.x.syncend,trymode.x.blankend,
	       			trymode.x.total,
	       trymode.y.width,trymode.y.blankstart,trymode.y.syncstart,
	       			trymode.y.syncend,trymode.y.blankend,
	       			trymode.y.total);
#endif

      if (ggiSetSimpleMode(vis, widthx, widthy, 1, GT_8BIT)) return;
      ...
      { 
        case MM_MOVE|GIIK_Left:
        	if (trymode.x.syncend+8<trymode.x.total) 
      		 { trymode.x.syncstart+=8;trymode.x.syncend+=8;
        ...
        case MM_BMOVE|GIIK_Right:
        	if (trymode.x.blankend  +8<=trymode.x.total &&
      		 ...
      		 { trymode.x.blankstart+=8;trymode.x.blankend+=8;}
      		 else beep();
      		break;
        ...
        case MM_BMOVE|GIIK_Down:
        	if (trymode.y.blankend  +1<=trymode.y.total &&
        	 ...
        	 { trymode.y.blankstart++;trymode.y.blankend++;}
        	 else beep();
        	break;
	...
	case 'H':
      ...
      switch(y)
      { case GIIK_Enter:oldmode=trymode;
      			break;
	case GIIK_Left:
	case GIIK_Right:
	case GIIK_Up:
	case GIIK_Down:
        case 27: break;
	case 'h':
	case 'H': /* Display help ... */  break;
        case 'm':
        case 'M':mmode=MM_MOVE;break;
        case 'r':
        case 'R':mmode=MM_RESIZE;break;
        case 'p':
        case 'P':mmode=MM_POLARITY;break;
        case 's':
        case 'S':mmode=MM_SYNCWIDTH;break;
        case 'b':
        case 'B':mmode=MM_BMOVE;break;
        case 'e':
        case 'E':mmode=MM_BRESIZE;break;

        default:printf("Unknown key: 0x%x\n",y);goto retry;
      }
      if (y==GIIK_Enter||y==27) break;
    }
    if (ggi_ioctl(MONITOR_SETPRESETMODE,&oldmode)) { perror("setpresetmode : "); }
}
...
{ int x;
  ...
  if (ggi_ioctl(MONITOR_CLEARPREF,NULL)) 
    ggiPanic("Clear preferred modes failed.Is the timelist-driver loaded ?");
...
{ 
  ...
  if (ggi_ioctl(MONITOR_SETPRESETMODE,&prefmode[0])) { perror("setprefmode : "); }

  if (ggiSetSimpleMode(vis, widthx, widthy, 1, GT_8BIT)) return 1;
  if (x<400) x*=2;if (y<300) y*=2;
