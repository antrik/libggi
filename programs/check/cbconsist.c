...
ggi_pixel cbconsist(cbcstate *s) {
  ggi_pixel bad;
  ggi_coord box;
  ggi_pixel count;

  box = s->smode.virt;
  if (box.x > s->dmode.virt.x) box.x = s->dmode.virt.x;
  if (box.y > s->dmode.virt.y) box.y = s->dmode.virt.y;
  bad = 0;
  ...
  do {
    ...
    while (num--) {
      ...
      if (s->flags & CBC_FLUSHALOT) ggiFlush(s->svis);
      curr--;
    }
    ...
    while (num--) {
      ...
      ggiGetPixel(s->dvis, num % box.x, num / box.x, &res);
	...
	if (s->flags & CBC_ABORT) {
	  if (curr == 0) {
	    ...
	    fprintf(stderr, "Kludge -- error is actually at 0.\n");
	    return 1;
	  }
	  return curr;
	}
...
int mkmemvis(int i, char **str, 
  ...
  if (i > MAX_MEMVIS_FMTS - 1) return -1;
  *str = memvis_fmts[i];
  *vis = ggiOpen(*str);
  if (!*vis) {
    fprintf(stderr, "Could not open memory source visual (%s)!\n", *str);
    return -1;
  }
  ggiSetFlags(*vis, GGIFLAG_ASYNC);
  if (ggiCheckSimpleMode(*vis,GGI_AUTO,GGI_AUTO,1,memvis_gts[i], mode)) {
    fprintf(stderr, "Could not find memory visual mode for gt %x!\n", 
	    memvis_gts[i]);
    return -1;
  }
  if (ggiSetMode(*vis, mode)) {
    fprintf(stderr, "Could not set up memory visual mode with gt %x!\n", 
	    memvis_gts[i]);
    return -1;
  }
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt(argc, argv, optstring)) != EOF) {
    ...
    default:
      if (strchr(optstring, optopt))
        fprintf(stderr, PROGNAME": option -%c requires"
                " an argument\n", optopt);
      else
  ...
  if (s.flags & CBC_REALSRC) {
    ...
    ggiCheckSimpleMode(s.svis,GGI_AUTO,GGI_AUTO,1,GT_AUTO, &s.smode);
  ...
  if (s.flags & CBC_REALDST) {
    ...
    ggiCheckSimpleMode(s.dvis,GGI_AUTO,GGI_AUTO,1,GT_AUTO, &s.dmode);
 ...
 err4:
  if (!(s.flags & CBC_REALDST)) ggiClose(s.dvis);
  if (!(s.flags & CBC_REALSRC)) ggiClose(s.svis);
  goto err1;
