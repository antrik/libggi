/* $Id: readpcx.c,v 1.1 2001/05/12 23:03:42 cegger Exp $
******************************************************************************
...
int readPCX (char *name, struct raw_pict *rp, uint32 udepth) {
   ...
   if ( (f = fopen(name,"rb")) ) {
	 ...
	 default:	bpp = 1; break;
      ...
      if ( (header.bits_per_pixel != 8) ||  ((cpl != 1) && (cpl != 3)) )
         err = RPREAD_BADFMT;
      ...
      if ((!err) && (fbuf = (uint8 *) malloc (width * height * bpp)) ) {
         ...
         if ( fgetc (f) == 12 ) {
            if ( (palette = (ggi_color *) malloc (256 * sizeof(ggi_color))) ) {
               for (i = 0; i < 256; i++) {
                  fread (palbuf, 1, 3, f);
                  palette[i].r = palbuf[0] << 8;
                  palette[i].g = palbuf[1] << 8;
                  palette[i].b = palbuf[2] << 8;
               }
               rp->clut = palette;
            }
