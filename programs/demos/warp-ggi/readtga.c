/* $Id: readtga.c,v 1.1 2001/05/12 23:03:45 cegger Exp $
******************************************************************************
...
static inline void StoreBPP(uint8 * data, int value, int bpp) {
  memcpy(data, &value, bpp);
}

static inline int RGBconv(int r, int g, int b, int bpp) {
  switch (bpp) {
  case 4:
  case 3:
    return (r << 16) | (g << 8) | b;
...
int readTGA(char *name, struct raw_pict *rp, uint32 udepth) {
  ...
  for (y=0; y<H && sl != end_p; y++) {
	...
	c = RGBconv(bgra[2], bgra[1], bgra[0], bpp);
	StoreBPP(sl, c, bpp);
	sl += bpp;
	x++;
