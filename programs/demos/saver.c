...
void blank_screen2(int interactive)
{
	...
	if (ggiSetSimpleMode(visual, GGI_AUTO, GGI_AUTO, GGI_AUTO, GT_8BIT)) {
	...
	while(1) {
		for(c=0;c<ysize;c++) {
			ggiSetGCForeground(visual,c/2+1);
		...
		c=ggiGetc(visual);
		switch(c) {
			case '+': if (timeout<1440) timeout++;
				break;
			case '-': if (timeout>   1) timeout--;
				break;
			case 'p': if (SaverActive-SaverList>0) SaverActive--;
				break;
			case 'n': if ((SaverActive+1)->func) SaverActive++;
				break;
			case 't': ggiSetGCForeground(visual,0);
				ggiFillscreen(visual);do_saver();
				break;
			case '\x1b':
			case 'q':
				ggiSetGCForeground(visual,0);
...
int main(int argc,char *argv[])
{
	...
	while(1) { 
		...
		if ((ic2=getic())==ic) {
			if (++cnt>=timeout) { 
				if ((visual=ggiOpen(NULL))==NULL) { 
					ggiPanic("cannot open default visual.\n");
				}

				if (ggiSetSimpleMode(visual, GGI_AUTO,GGI_AUTO,
						     GGI_AUTO, GT_8BIT)) {
					ggiPanic("cannot set 8bpp mode.");
				} else {
					ggi_mode mode;
