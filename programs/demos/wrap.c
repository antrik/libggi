...
void init_client(client_t *client, ggi_mode *mode, const char *command)
{
  ...
  if (client->sockfd == -1) fprintf(stderr, "error in socket %s\n", strerror(errno));  
   ...
   */
  client->semid = 0;
  bufsize = mode->virt.x * mode->virt.y*((GT_SIZE(mode->graphtype) + 7) / 8) + 64 * 1024;
  ...
  if ((client->visual = ggiOpen(text, NULL)) == NULL) { ggiPanic("Ouch - can't open the shmem target !");}
  mode->frames = 1;
   ...
   * start the program
   */
  sprintf(text, "display-ipc:-input -socket=%s -semid=%d -shmid=%d", client->socket, client->semid, client->shmid);
