...
static Bool _ggi_XF86DGADirectVideoLL(Display* dpy, int screen, int enable)
{
    XExtDisplayInfo *info = find_display (dpy);
...
int _ggi_XF86DGADirectVideo(Display *dis, int screen, int enable)
{
   if (enable&XF86DGADirectGraphics) {
      ...
      if (_XFree86addr && _XFree86size)
         ...
         fprintf(stderr, "_ggi_XF86DGADirectVideo: mprotect (%s)\n",
                           strerror(errno));
		exit (-3);
	 }
#endif
			 ...
			 int *width, int *bank, int *ram)
{
   ...
   if ((memory_fd = open(devname, O_RDWR)) < 0) {
        fprintf(stderr, "_ggi_XF86DGAGetVideo: failed to open %s (%s)\n",
   ...
   if ((*addr = (char *) ioctl( memory_fd, MAP, &XFree86mloc)) != (char *)-1) {
     offset=(int)XFree86mloc.physaddr;
     *bank=(int)XFree86mloc.length;
   } else {
     fprintf(stderr, "_ggi_XF86DGAGetVideo: failed to mmap /dev/mmap (%s)\n",
                           strerror(errno));
#else /* !ISC */
#ifdef Lynx
   *addr = (void *)smem_create("XF86DGA", (char *)offset, *bank, SM_READ|SM_WRITE);
   if (*addr == NULL) {
        fprintf(stderr, "_ggi_XF86DGAGetVideo: smem_create() failed (%s)\n",
                           strerror(errno));
#else /* !Lynx */
#ifndef MAP_FILE
#define MAP_FILE 0
   ...
   *addr = (void *)mmap(NULL, *bank, PROT_READ,
                        MAP_FILE | MAP_SHARED, memory_fd, (off_t)offset);
   if (*addr == MAP_FAILED) {
        fprintf(stderr, "_ggi_XF86DGAGetVideo: failed to mmap %s (%s)\n",
                           devname, strerror(errno));
#endif /* !Lynx */
